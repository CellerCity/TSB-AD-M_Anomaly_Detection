"""
LSTMAD helper for F2A anomaly detection benchmark.

Protocol: fit on data_train, score on data_test, evaluate against labels_test.

LSTMAD is an LSTM encoder-decoder forecaster. Per-timestep anomaly scores
are squared forecast errors averaged across features.

Hyperparameters: from TSB_AD/HP_list.py -> Optimal_Multi_algo_HP_dict['LSTMAD'].
The LSTMAD class hardcodes EarlyStoppingTorch(patience=3) internally and does
...
if not hasattr(LSTMAD, "__sklearn_tags__"):
    from sklearn.base import BaseEstimator
    LSTMAD.__sklearn_tags__ = BaseEstimator.__sklearn_tags__
...
def anomaly_LSTMAD(data_train: np.ndarray,
                   data_test: np.ndarray,
                   labels_test: np.ndarray,
                   sliding_window: int):
    """
    Fit LSTMAD on data_train and evaluate on data_test.

    Note: sliding_window here is used ONLY for get_metrics range-based scoring.
    The model's own sequence length is LSTMAD_HP['window_size'] = 100,
    ...
    window_size = LSTMAD_HP["window_size"]
    pred_len = LSTMAD_HP["pred_len"]

    # ForecastDataset needs window_size + pred_len rows in each split
    min_train = int(np.ceil((window_size + pred_len) / (1 - LSTMAD_HP["validation_size"])))
    if data_train.shape[0] < min_train:
        raise ValueError(
            f"Train too short ({data_train.shape[0]} rows) for "
            f"window_size={window_size}, pred_len={pred_len}, "
            f"validation_size={LSTMAD_HP['validation_size']}. Need >= {min_train}."
        )
    ...
    with _suppress_output():
        detector = LSTMAD(
            ...
            pred_len=pred_len,
            batch_size=LSTMAD_HP["batch_size"],
            epochs=LSTMAD_HP["epochs"],
            lr=LSTMAD_HP["lr"],
            feats=feats,
            hidden_dim=LSTMAD_HP["hidden_dim"],
            num_layer=LSTMAD_HP["num_layer"],
            validation_size=LSTMAD_HP["validation_size"],
        )
